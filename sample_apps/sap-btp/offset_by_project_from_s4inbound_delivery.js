...
import '@sap-cloud-sdk/odata-v2';
import { InbDeliveryItemApi } from "./odata-clients/OP_API_INBOUND_DELIVERY_SRV_0002/InbDeliveryItemApi";
...
// SAP BTP uses concept of 'destination' - URL and chosen authentication bundled together
// See https://sap.github.io/cloud-sdk/docs/js/features/connectivity/destinations for more details
var SAP_S4_DESTINATION = {}    
SAP_S4_DESTINATION.name = "S4HANATEST";
SAP_S4_DESTINATION.url = process.env.SAP_BASE_URL + "/sap/opu/odata/sap/API_INBOUND_DELIVERY_SRV;v=0002";
SAP_S4_DESTINATION.username = process.env.SAP_USER;
SAP_S4_DESTINATION.password = process.env.SAP_PW;

const s4DeliveryAPI = InbDeliveryItemApi();

// TIP use the filter criteria
// https://sap.github.io/cloud-sdk/docs/js/features/odata/v4-client#getall-request-builder
const inboundDeliveryItem = await s4DeliveryAPI.InbDeliveryItemTextRequestBuilder().getAll().top(1).execute(SAP_S4_DESTINATION);

console.log(inboundDeliveryItem);
...
// setup the retirement request ~ use the BTP data for emissions to offset!

/*
var emissionsAmount = productFootprints[0].totalCo2EQuantity;

switch( productFootprints[0].totalCo2EQuantityUoM ) {
    case 'KG':
        emissionsAmount = emissionsAmount * 1000;
        break;
    default:
